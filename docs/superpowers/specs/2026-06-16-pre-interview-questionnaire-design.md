...
> Bridge（Claude_Resume_Check）の「質問生成」ページに、面接前に候補者へ送る WEB アンケートを実用化する機能を追加する。
> 方針：**Vercel + Upstash Redis を用いた自前アンケート基盤**。本地 Bridge がコンテンツを生成・配信制御、公開アンケートと回答保管は Vercel 上の軽量サービスが担う。

- 作成日：2026-06-16
- 対象範囲：
  - 本地：Bridge 内 `public/index.html` の「質問生成」ページ＋ `server.js` の関連 API
...
- Bridge は履歷から人物像を要約、要件＋人物像から面接質問を生成できる
...
④【新規】HR が質問を編集（追加 / 削除 / 文言調整）
   ↓
⑤【新規】「アンケートを公開」ボタン
   → Bridge が Vercel デプロイの公開 API に POST：候補者専用 URL を発行
   → 7 日後に自動失効、単回提出
   ↓
...
⑦候補者が URL を開く → 自前アンケートページで回答
   → 提出 → Vercel KV に保存
   ↓
⑧【新規】Bridge「回答取得」ボタン or 5 分自動ポーリング
   → 本地 answers.json
   ↓
...
- 業務上「Google ログインさせたくない」「UI を Bridge 系統と一致させたい」「単回提出 / 7 日失効を完全制御したい」「将来 BridgeVC ブランドでカスタマイズしたい」
...
- 公開側：Next.js（App Router）アプリ、Vercel デプロイ。**初期は Vercel 既定ドメイン**（例 `bridge-survey-xxx.vercel.app`）を使用。将来カスタムドメインに切替可能
- データ層：Upstash Redis（Vercel Marketplace 経由）
- 本地：質問編集 UI、配信ボタン、ステータス管理、回答取得 UI（手動 + 自動ポーリング）
- 認証：Bridge ⇔ Vercel 間は共有秘密キー（API Key）。候補者は無認証、URL トークンのみ
...
│   .bridge/survey-config.json  Vercel エンドポ + API KEY │
└──────────────────────┬──────────────────────────┘
                       │ HTTPS + API Key
                       ↓
┌─────────────────────────────────────────────────┐
...
#### Bridge → Vercel
- 共有秘密キー `SURVEY_API_KEY`（32 字 random hex）を `.bridge/survey-config.json` に保存
...
「アンケートを公開」を押すとモーダルが開く。Bridge → Vercel API でトークン発行。
...
- 構造：
  ```json
  {
    ...
    "surveyApiKey": "<本ファイルではなく .bridge/survey-config.json に分離>",
    "emailTemplate": {
      "subject": "...",
      "body": "..."
    },
  ...
  ```
- **API Key は別ファイル**（`.bridge/survey-config.json`、`.gitignore` 対象）に分離。`presets/settings.json` は誤って Git に入っても秘密漏洩しない

---

## 5. 公開側設計（Vercel `bridge-survey`）

### 5.1 技術スタック

- Next.js 14+（App Router）
- TypeScript
- Tailwind CSS（Bridge 視覚言語を踏襲）
...
Bridge → Vercel POST `/api/surveys`、結果を `questions/{id}.json` に記録、`status = "sent"`。
...
- Bridge → Vercel フロー：dispatch → fetch → close（モック Vercel API）
...
8. デプロイ → Vercel が割り当てた URL（例 `bridge-survey-xxx.vercel.app`）をコピー
9. Bridge 設定ページの「Vercel アンケート連携」に URL と API Key を登録
...
- Bridge 設定ページの API エンドポイントを新ドメインに更新

### 10.5 Bridge 側設定ファイル

- `.bridge/survey-config.json`：
  ```json
  {
  ...
  ```
- `.gitignore` に `.bridge/` を追加（API Key を含むため）
